...
#define FONTNAME "BIOS_F16"
#define NCHARS (256)

/*************************************************************************************************/

uint8_t bitmap[NCHARS * 16];

int main()
{
    // Prepare bitmaps & blocks
    FILE *fontx = fopen("BIOS_F16.fnt", "wb");
    ...
    header[15] = 16u;      // Font height HF (dots)
    header[16] = 0u;      // Type ANK (single byte)
    fwrite(&header, sizeof(header), 1, fontx);

    // FONTX2 BITMAP
    fwrite(&BIOS_F16, sizeof(BIOS_F16), 1, fontx);
